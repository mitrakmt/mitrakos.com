        ...
        >
          {videos.map((video) => (
            <li key={video.title} className="[counter-increment:video]">
              ...
              <h3 className="mt-8 text-base font-medium tracking-tight text-slate-900 dark:text-slate-100 before:mb-2 before:block before:font-mono before:text-sm before:text-slate-500 before:content-[counter(video,decimal-leading-zero)]">
                {video.title}
              </h3>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{video.description}</p>
            </li>
          ))}
        </ol>
      </Container>
